...
import blog1 from "../../assets/models/blog-detail-2.jpg";
...
import CardWithoutBtn from "../ListingCard/ListingCardWithoutBtn";
import { useRouter } from "next/router";
import { SITE_URL } from "@/config/constants";
import { formatBlogDate,getBlogTags , calculateReadTime} from "@/helpers/helper";
import listingService from "@/services/listingService";

function BlogDetail({blog}) {
  const eliteItems = [{ img: elite1 }, { img: elite2 }, { img: elite1 }];
  
  const [famousTherapists, setFamousTherapists] = useState([]);
 const router = useRouter();
  ...
  const fetchFamousListing = async () =>{
    try {
      
      var data = null;
      var filter = {status:'Active',limit:8,famous_only:"YES"}
      
      data = await listingService.list(filter);
     
      if(data){
        setFamousTherapists(data.data);
      }
    } catch (error) {
  ...
  return (
    <div className="flex justify-center bg-[#FCFCFD]">
      <div className="max-w-[1440px] w-full">
        <div className=" py-[32px] md:py-10 px-6 md:px-[64px] gap-10 grid grid-cols-1 lg:grid-cols-3">
          <div className="md:col-span-2 flex flex-col gap-[60px] md:gap-12 lg:mr-[93px]">
            <div className="flex flex-col gap-16">
              ...
              <div className="flex flex-col md:flex-row items-start md:items-end justify-between gap-4 md:gap-0">
                <div className="flex flex-col gap-4">
                  <div className="fs-18-700-lato lh-27 txt-color-gray700">
                    Share this post
                  </div>
                ...
                (
                  ...
                  {getBlogTags(blog?.tags).map((tag,index)=>(
                    <div className="p-2 rounded-lg bg-[#F2F4F7] fs-14-500-lato txt-color-red" key={index}>
                      {tag}
                    </div>
                  ))}
                  </div>
            ...
            <div className="md:hidden border-t border-[#EAECF0]"></div>
          </div>
          {/* Right side of blog detail  */}
          <div className="mt-8 md:mt-0 p-0 md:p-8 rounded-lg bg-transparent md:bg-white flex flex-col gap-8 h-fit">
            <div className="fs-32-400-lato txt-color-red">
              Famous Therapists 
            </div>
           
            {famousTherapists?.map((item, index) => (
              <CardWithoutBtn item={item} pic={item?.img} key={index} topIcon={0} />
            ))}
